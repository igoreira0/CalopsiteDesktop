...
namespace Calopsite
{
    public partial class Alimentacao : Form
    {
        ...
        private Hist_Alimentacao histAlimentacao = new Hist_Alimentacao();
        public Alimentacao(int ID_Login)
        {
            InitializeComponent();
            login.id_usuario = ID_Login;
        }

        private void Aimentacao_Load(object sender, EventArgs e)
        {
            loadGaiolas();
            LoadRacao();
        }
        private void LoadRacao()
        {
            BD bd = new BD();
            StringBuilder str = new StringBuilder("SELECT ID_Insumo, Nome FROM Insumo WHERE Quantidade IS NULL");
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                histAlimentacao.id_racao = int.Parse(comboBox2.SelectedValue.ToString());
                histAlimentacao.peso = float.Parse(textBox1.Text);
                histAlimentacao.id_gaiola = gaiola.id_gaiola;
            }catch(Exception)
            {
                MessageBox.Show("Valor Invalido");
            }
            BD bd = new BD();
            StringBuilder str = new StringBuilder("INSERT INTO Hist_Alimentacao (ID_Racao, Peso, ID_Gaiola) VALUES (@ID_Racao, @Peso, @ID_Gaiola)");
            MySqlCommand command = new MySqlCommand(str.ToString());
            command.Parameters.Add("@ID_Racao", MySqlDbType.Int32);
            command.Parameters["@ID_Racao"].Value = histAlimentacao.id_racao;
            command.Parameters.Add("@Peso", MySqlDbType.Float);
            command.Parameters["@Peso"].Value = histAlimentacao.peso;
            command.Parameters.Add("@ID_Gaiola", MySqlDbType.Int32);
            command.Parameters["@ID_Gaiola"].Value = histAlimentacao.id_gaiola;
            try
            {
                bd.executarComando(command);
                MessageBox.Show("Alimentação realizada com sucesso");
            }
            catch (Exception)
            {
